        ...
        {cards.map((card) => {
          ...
          return (
            <div key={card.label} className="bg-[#111118] border border-[#2a2a3a] rounded-xl p-5 flex flex-col gap-3 hover:border-[#3a3a4a] transition-all">
              <div className={`w-9 h-9 ${card.bg} flex items-center justify-center rounded-lg`}>
      ...
      {stats?.recent_posts?.length > 0 && (
        <div className="bg-[#111118] border border-[#2a2a3a] rounded-xl overflow-hidden">
          ...
          {stats.recent_posts.map((post: any, i: number) => (
            <div key={post.id}
              className={`flex items-center justify-between gap-4 px-6 py-4 hover:bg-[#1a1a24] transition-all ${i !== stats.recent_posts.length - 1 ? 'border-b border-[#2a2a3a]' : ''}`}>
              <div className="flex items-center gap-3 min-w-0">
                <span className={`w-2 h-2 rounded-full flex-shrink-0 ${post.status === 'published' ? 'bg-green-400' : 'bg-yellow-400'}`} />
