    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white">
      <header className="border-b border-[#2a2a3a] bg-[#111118]/80 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <Link to="/login" className="flex items-center gap-1.5 text-xs font-mono px-3 py-1.5 border border-[#2a2a3a] rounded-lg text-[#888899] hover:text-white hover:border-[#7c6aff] transition-all"><Lock size={12} />Admin</Link>
        </div>
      </header>
        ...
        <div className="flex flex-col gap-px border border-[#2a2a3a] rounded-xl overflow-hidden">
          {posts.map((post: any, i: number) => (
            <Link key={post.id} to={`/blog/${post.slug}`}
              className={`flex items-start justify-between gap-6 p-6 bg-[#111118] hover:bg-[#1a1a24] transition-all group
                ${i !== posts.length - 1 ? 'border-b border-[#2a2a3a]' : ''}`}>
